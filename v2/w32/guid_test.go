...
func TestGUID(t *testing.T) {
	for i := 0; i < len(guidFixtures); i++ {
		guid := NewGUID(guidFixtures[i].String)
		f := guidFixtures[i]
		if guid == nil {
			if f.Guid != nil {
				t.Errorf("GUID test \"%v\" (%v of %v) failed. Expected %v from NewGUID. Received <nil> instead.", f.Name, i, len(guidFixtures), f.Guid)
			}
		} else if f.Guid == nil {
			t.Errorf("GUID test \"%v\" (%v of %v) failed. Expected <nil> from NewGUID. Received %v instead.", f.Name, i, len(guidFixtures), guid)
		}
		if guid == nil || f.Guid == nil {
			continue
		}
		if !guid.Equal(f.Guid) {
			t.Errorf("GUID test \"%v\" (%v of %v) failed. Expected %v from NewGUID. Received %v instead.", f.Name, i, len(guidFixtures), f.Guid, guid)
		}
		if f.ShouldMatch && guid.String() != strings.ToUpper(f.String) {
			t.Errorf("GUID test \"%v\" (%v of %v) failed. Expected \"%v\" from GUID.String. Received \"%v\" instead.", f.Name, i, len(guidFixtures), strings.ToUpper(f.String), guid)
		}
	}
}
